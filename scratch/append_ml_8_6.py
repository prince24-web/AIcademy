...
lesson_8_6_code = """  'ml-8-6': {
    ...
    sections: [
      {
        id: 'section-1',
        title: '1. The Sanctity of the Pristine Holdout Test Set & Probability Calibration',
        content: [
      ...
      {
        id: 'section-2',
        title: '2. Cell 21: Full Pipeline Training & Holdout Test Calibration Audit',
        content: [
          ...
          'We fit the complete pipeline on all 5,634 training records, and then for the very first time, execute inference across the pristine 1,409 holdout test records.'
        ],
        codeBlock: {
          language: 'python',
          fileName: 'cell_21_retrain_and_holdout_eval.py',
          code: `# Cell 21: Full Pipeline Training & Holdout Test Evaluation
import pandas as pd
import numpy as np
...
print(f"[HOLDOUT TEST BENCHMARKS - 1,409 SUBSCRIBERS]")
...
print(f"Overfitting Gap: {train_roc - test_roc:.4f} (Train: {train_roc:.4f})")`
        ...
        output: `[HOLDOUT TEST BENCHMARKS - 1,409 SUBSCRIBERS]
...
Net Financial Savings vs Baseline: +$317,300`,
      ...
      {
        id: 'section-3',
        title: '3. Cell 22: Asymmetric Cost-Utility Threshold Optimization',
        content: [
        ...
        codeBlock: {
          language: 'python',
          fileName: 'cell_22_threshold_optimization.py',
          code: `# Cell 22: Asymmetric Threshold Optimization Sweep
import numpy as np
import pandas as pd
...
print(f"Annual Financial Lift:   +\${int(best_balanced['savings'] - 317300):,} in recovered profit!")`
        ...
        output: `[THRESHOLD OPTIMIZATION RESULTS]
...
Annual Financial Lift:   +$178,800 in recovered profit!`,
      ...
      {
        id: 'section-4',
        title: '4. Cell 23: Production Pipeline Serialization & Smoke Testing',
        content: [
        ...
        codeBlock: {
          language: 'python',
          fileName: 'cell_23_pipeline_serialization.py',
          code: `# Cell 23: Production Pipeline Serialization & Smoke Test
import os
import hashlib
import joblib
import pandas as pd
...
print(f"[ARTIFACT PACKAGING METRICS]")
...
print(f"Payload 2 (2-Yr Loyal DSL): Churn Prob = {prob_loyal*100:.2f}% -> Decision: {action_loyal}")`
        ...
        output: `[ARTIFACT PACKAGING METRICS]
...
Payload 2 (2-Yr Loyal DSL): Churn Prob = 2.41% -> Decision: STANDARD CARE`,
      ...
      {
        id: 'section-5',
        title: '5. Cell 24: Enterprise Model Card Generation & Subgroup Fairness Audit',
        content: [
        ...
        codeBlock: {
          language: 'python',
          fileName: 'cell_24_model_card_and_fairness.py',
          code: `# Cell 24: Subgroup Fairness & Demographic Parity Audit
import pandas as pd
from sklearn.metrics import confusion_matrix
...
audit_slice('Partner')`
        ...
        output: `--- Fairness Audit: gender (tau = 0.22) ---
...
--- Fairness Audit: Partner (tau = 0.22) ---
  ...
  Disparate Impact Ratio: 0.557 (80% Rule Compliance: FLAG)`,
      ...
      {
        id: 'section-6',
        title: '6. Pre-Deployment Checklist: Artifact Validation, CI/CD Packaging & Production Handoff',
        content: [
...
# Replace the closing '  }\n};' of mlLessonsData with the new lesson
marker = "    quiz: {\n      question: 'Why does Tree-structured Parzen Estimator (TPE)"
pos = content.find(marker)
if pos == -1:
    print("Could not find marker for ml-8-5 quiz!")
    exit(1)

# Find closing of ml-8-5
end_of_8_5 = content.find("  }\n};", pos)
if end_of_8_5 == -1:
    print("Could not find closing of ml-8-5!")
    exit(1)

new_content = content[:end_of_8_5] + "  },\n\n" + lesson_8_6_code

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(new_content)

print(f"Successfully appended ml-8-6 to {file_path}")
