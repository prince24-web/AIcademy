...
- \`POST /predict\`: Single subscriber inference applying optimal threshold tau* = 0.22.
...
- **Do-Nothing Annual Loss**: 2,654 x 1,200 USD LTV = **3,184,800 USD**
...
- **Promotional Voucher Expense**: 100 USD per flagged subscriber.

We compare the tuned champion model (tau* = 0.22) directly against the un-tuned default model (tau = 0.50):`,
        codeBlock: {
          language: 'python',
...
When the Population Stability Index (PSI) radar flags significant covariate drift (PSI > 0.25), an automated pipeline:
