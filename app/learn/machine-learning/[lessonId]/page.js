...
// ─── CODDY-STYLE INTERACTIVE CODING WORKSPACE (MATCHING PYTHON 10-1) ────────
const MLInteractiveCodingWorkspace = ({ lessonId, lesson, prevLessonId, nextLessonId }) => {
  ...
  // Dynamically load Pyodide WebAssembly runtime
  useEffect(() => {
...
import sys, io
_ml_stdout_capture = io.StringIO()
_ml_stderr_capture = io.StringIO()
sys.stdout = _ml_stdout_capture
sys.stderr = _ml_stderr_capture
`);

        try {
          await py.runPythonAsync(code);
          const stdoutText = await py.runPythonAsync(`_ml_stdout_capture.getvalue()`);
          const stderrText = await py.runPythonAsync(`_ml_stderr_capture.getvalue()`);
          ...
          if (lesson.expectedOutput && combinedOutput === lesson.expectedOutput.trim()) {
            setIsPassed(true);
            triggerConfetti(0.6, 0.5);
          } else if (!lesson.expectedOutput && combinedOutput) {
            setIsPassed(true);
          } else {
            setIsPassed(false);
          }
...
`);
        ...
        setTimeout(() => {
          let simulatedOut = 'Output generated';
          ...
          setIsRunning(false);
          if (lesson.expectedOutput && simulatedOut.trim() === lesson.expectedOutput.trim()) {
            setIsPassed(true);
            triggerConfetti(0.6, 0.5);
          }
        }, 400);
    ...
    <div className={styles.codingContainer}>
      ...
      <header className={styles.topHeader}>
        <div className={styles.headerLeft}>
          <Link href="/learn/machine-learning" className={styles.btnClose} title="Back to Roadmap">
            ✕
          </Link>
          <div className={styles.navGroup}>
            {prevLessonId ? (
              <Link href={`/learn/machine-learning/${prevLessonId}`} className={styles.navBtnHeader}>
                ‹ Prev
              </Link>
            ) : (
              <button disabled className={styles.navBtnHeaderDisabled}>‹ Prev</button>
            )}
            {nextLessonId ? (
              <Link href={`/learn/machine-learning/${nextLessonId}`} className={styles.navBtnHeader}>
                Next ›
              </Link>
            ) : (
              <button disabled className={styles.navBtnHeaderDisabled}>Next ›</button>
            )}
          </div>
          <span className={styles.headerLessonTitle}>{lesson.title}</span>
        </div>

        <div className={styles.headerRight}>
          <div className={styles.statItem}>
            <StatIconStreak size={18} />
            <span className={`${styles.statText} ${styles.statFlame}`}>0</span>
          </div>
          <div className={styles.statItem}>
            <StatIconGem size={18} />
            <span className={`${styles.statText} ${styles.statGems}`}>20</span>
          </div>
          <div className={styles.statItem}>
            <StatIconEnergy size={18} />
            <span className={`${styles.statText} ${styles.statEnergy}`}>5</span>
          </div>
      ...
      <div className={styles.workspace}>
        ...
        {/* LESSON CONTENT PANEL (LEFT SPLIT) */}
        <section className={styles.lessonPanel}>
          <div className={styles.lessonHeader}>
            <div>
              <h1 className={styles.lessonMainTitle}>{lesson.subtitle || lesson.title}</h1>
              {lesson.gfgUrl && (
                <a href={lesson.gfgUrl} target="_blank" rel="noopener noreferrer" className={styles.gfgRefLink}>
          ...
          {lesson.paragraphs && lesson.paragraphs.length > 0 ? (
            ...
            <p className={styles.lessonParagraph}>{lesson.description || ''}</p>
          )}

          {/* Real-World Analogy Card */}
          {lesson.analogy && (
            <div className={styles.analogyCardWorkspace}>
              <div className={styles.analogyTitleWorkspace}>
                <IconLightbulb size={20} />
                <span>Real-World Analogy</span>
              </div>
              <p className={styles.analogyTextWorkspace}>
                {typeof lesson.analogy === 'string' ? lesson.analogy : lesson.analogy.text}
              </p>
            </div>
          )}
          ...
          {lesson.codeExample && (
            <div className={styles.exampleBlock}>
              ...
              <pre
                ...
                dangerouslySetInnerHTML={{ __html: highlightPython(lesson.codeExample) }}
              />
            </div>
          )}
          ...
          {lesson.takeaways && lesson.takeaways.length > 0 && (
            <div className={styles.takeawaysBox}>
              <div className={styles.takeawaysBoxTitle}>Key Rules &amp; Concepts</div>
              <ul className={styles.takeawaysBoxList}>
                {lesson.takeaways.map((take, idx) => (
                  <li key={idx}>{take}</li>
                ))}
              </ul>
            </div>
          ...
          {/* Challenge Box */}
          {(lesson.challengeIntro || (lesson.instructions && lesson.instructions.length > 0)) && (
            <div className={styles.challengeCard}>
              ...
              {lesson.challengeIntro && (
                <p className={styles.challengeIntro}>{lesson.challengeIntro}</p>
              )}
          ...
          <div className={styles.lessonNavFooter}>
            {prevLessonId ? (
              <Link href={`/learn/machine-learning/${prevLessonId}`} className={styles.btnLessonNavPrev}>
                ‹ Previous Lesson
              </Link>
            ) : <div />}

            {nextLessonId ? (
              <Link href={`/learn/machine-learning/${nextLessonId}`} className={styles.btnLessonNavNext}>
                Next Lesson ›
              </Link>
            ) : (
              <Link href="/learn/machine-learning" className={styles.btnLessonNavNext}>
                Finish Module
              </Link>
            )}
          </div>
        ...
        <section className={styles.editorPanel}>
          ...
          {/* Code Editor with Real Python Syntax Highlighting */}
          ...
          <div className={styles.outputPanel}>
            {isPassed && (
              <div className={styles.successBanner}>
                ...
                {nextLessonId ? (
                  <Link href={`/learn/machine-learning/${nextLessonId}`} className={styles.btnContinueNext}>
                    Continue to Next Lesson →
                  </Link>
                ) : (
                  <Link href="/learn/machine-learning" className={styles.btnContinueNext}>
                    Module Completed
                  </Link>
                )}
              </div>
            ...
            <div className={styles.outputSplit}>
              ...
              <div className={styles.expectedBox}>
                ...
                <div>{lesson.expectedOutput || '(Run code to see output)'}</div>
              </div>
            </div>
          </div>
...
export default function MLLessonArticlePage() {
  ...
  // If this lesson is an interactive coding workspace (Module 2 coding track)
  if (lesson.isCodingLesson) {
    return (
      <MLInteractiveCodingWorkspace
        lessonId={lessonId}
        lesson={lesson}
